...
import {
  ...
  HeaderRenderScope,
  ProTableInstance,
} from "packages/ProTable/interface";
...
const shouldUpdate = (newObj, oldObj) => {
  if (newObj?.user234 != oldObj?.user234) {
    return true;
  }
  return false;
};
    ...
    >
      ...
      <template #tableHeader="scope">
        ...
        <el-button
          ...
          @click="deleteUser(scope.selectedListIds)"
        >
          删除用户
        </el-button>
        <el-button :icon="More" @click="selectedData(scope.selectedList)"
          >选中数据</el-button
        >
      </template>
      <!-- Expand -->
